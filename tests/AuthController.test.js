...
describe('+ AuthController', () => {
    ...
    describe('+ GET: /connect', () => {
        ...
        it('+ Fails with a invalid email and valid password', function (done) {
            this.timeout(5000);
            request.get('/connect')
                .auth('invalid@email.com', mockUser.password, { type: 'basic' })
                .expect(401)
                .end((err, res) => {
                    if (err) {
                        return done(err);
        ...
        it('+ Succeeds for existing user', function (done) {
            this.timeout(5000);
            request.get('/connect')

...
describe('+ RedisClient utility', () => {
    ...
    it('+ Setting and getting a value', async function () {
        await redisClient.set('test_key', 340, 10);
        expect(await redisClient.get('test_key')).to.equal('340');
    });

    it('+ Setting and getting an expired value', async function () {
        await redisClient.set('test_key', 366, 1);
        setTimeout(async () => {
            expect(await redisClient.get('test_key')).to.equal('366');
        }, 2000);
    });
